-- chunkname: @./kr5/screen_kr5_end.lua
...
function screen:next_item()
	...
	if self.phase == "comic7" then
		S:queue("MusicCredits", {
			loop = false
		})

		screen_comics.comic_data = love.filesystem.read(director:get_comic_data_file(7))
	...
	elseif self.phase == "credits" then
		...
		self.phase = "comic8"
	elseif self.phase == "comic8" then
		S:stop_all()
		S:queue("MusicSuspense")

		screen_comics.comic_data = love.filesystem.read(director:get_comic_data_file(8))
