...
local scripts = require("scripts_v")
...
function scripts.jt_spawner_aura.update(this, store)
	...
	if not owner then
		...
		while owner and not owner.dying and not owner.health.dead do
			for i, v in ipairs(this.spawn_data) do
				...
				if store.tick_ts - spawn_ts[i] >= cooldown + delay then
					...
					e.nav_path.pi = pi
					e.nav_path.spi = spi or math.random(1, 3)
...
function scripts.aura_ignus_particles.remove(this, store)
	for _, p in pairs(this.particles) do
...
function scripts.aura_ignus_particles.update(this, store)
	...
	for i, o in ipairs(this.particle_offsets) do
		...
		queue_insert(store, ps)
		table.insert(this.particles, ps)
	end
...
function scripts.aura_ignus_surge_of_flame.update(this, store)
	...
	while s.name == this.damage_state do
		if store.tick_ts - ts + 1e-09 <= a.cycle_time then
			...
			targets = U.find_soldiers_in_range(store.entities, this.pos, 0, a.damage_radius, a.damage_flags or 0, a.damage_bans or 0) or {}
			ts = store.tick_ts
				...
				for _, s in ipairs(g.spawns_by_health) do
					...
					if t.health.hp_max > s[2] then
						...
						if #nearest_nodes < 1 then
							...
							if not U.is_seen(store, s[1]) then
								signal.emit("wave-notification", "icon", s[1])
								U.mark_seen(store, s[1])
							end
...
function scripts.enemy_base_portal.insert(this, store)
	...
	if portal and this.nav_path.ni - portal.out_nodes[this.nav_path.pi] < 8 then
