...
function U.find_towers(entities, origin, min_range, max_range, filter_func)
	local l_towers = SSO and SSO:get_p_list("towers") or entities
	local towers = table.filter(l_towers, function(k, v)
		return not v.pending_removal and v.tower and not v.tower.blocked and U.is_inside_ellipse(v.pos, origin, max_range) and (min_range == 0 or not U.is_inside_ellipse(v.pos, origin, min_range)) and (not filter_func or filter_func(v, origin))
	end)
...
function U.find_towers_in_range(entities, origin, attack, filter_func)
	local towers = U.find_towers(entities, origin, attack.min_range, attack.max_range, function(v, o)
		return (not attack.excluded_templates or not table.contains(attack.excluded_templates, v.template_name)) and (not filter_func or filter_func(v, origin, attack))
	end)
...
function U.unlock_next_levels_in_ranges(unlock_data, levels, game_settings, unlocked_dlcs)
	...
	if expansions_unlock_level then
	...
	if unlocked_dlcs and dlcs_unlock_level then
