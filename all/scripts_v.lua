...
function scripts.enemy_basic.get_info(this)
	local min, max, attacks, minr, maxr, dodge_chance
	local ranged = nil
	local dodge = nil
	local damage_type_ranged = nil
			...
			if a.bullet then
				...
				if b and b.bullet.damage_min and b.bullet.damage_max then
					if a.damage_factor then
						minr, maxr = b.bullet.damage_min * a.damage_factor * this.unit.damage_factor, b.bullet.damage_max * a.damage_factor * this.unit.damage_factor
					else
						minr, maxr = b.bullet.damage_min * this.unit.damage_factor, b.bullet.damage_max * this.unit.damage_factor
					end
	...
	if minr and maxr then
		ranged = true
		minr, maxr = math.ceil(minr), math.ceil(maxr)
	end
	if this.dodge then
		dodge = true
	...
	local armor = this.health.armor
	local magic_armor = this.health.magic_armor
...
function scripts.enemies_spawner.update(this, store, script)
	...
	if not sp.pi then
		local path_indexes = sp.path_indexes
		local safe_nodes = sp.safe_nodes
		local nodes = P:nearest_nodes(this.pos.x, this.pos.y, path_indexes, nil, nil, nil, safe_nodes)
	...
	for i = 1, sp.count do
		...
		if sp.check_node_valid and not P:is_node_valid(e_pi, e_ni) then
			...
			if sp.fade_in then
				E:add_comps(spawn, "tween")
				spawn.tween.remove = false
				spawn.tween.run_once = true
				spawn.tween.props[1].name = "alpha"
				spawn.tween.props[1].keys = {
					{
						0,
						0
					},
	...
	elseif this.tween then
		if this.render.sprites[1].animated == true then
...
function scripts.soldier_reinforcement.get_info(this)
	local attacks, minr, maxr, dodge_chance
	local ranged = nil
	local dodge = nil
	local damage_type_ranged = nil
	local damage_type = nil
...
function scripts.soldier_reinforcement.update(this, store, script)
	...
	elseif this.render.sprites[1].name == "raise" then
		...
		U.animation_start(this, "raise", nil, store.tick_ts, false)
...
function scripts.hero_basic.get_info_ranged(this)
	...
	local am = this.melee.attacks[1]
	local b = E:get_template(a.bullet)
	local minr, maxr = b.bullet.damage_min, b.bullet.damage_max
	local min, max = am.damage_min, am.damage_max
	local damage_type = nil
	local dodge_chance
	local dodge = nil
	...
	minr, maxr = minr * this.unit.damage_factor, maxr * this.unit.damage_factor
...
function scripts.tower_barrack.get_info(this)
	...
	if s.ranged and s.ranged.attacks then
		for _, a in pairs(s.ranged.attacks) do
			if a.bullet then
				local b = E:get_template(a.bullet)

				if b and b.bullet.damage_min and b.bullet.damage_max then
					minr, maxr = b.bullet.damage_min, b.bullet.damage_max
...
function scripts.bolt.update(this, store, script)
	...
	if target and not target.health.dead and not SU.unit_dodges(store, target, true) then
		local d = SU.create_bullet_damage(b, target.id, this.id)
...
function scripts.shotgun.update(this, store, script)
	...
	if target and target.health and not target.health.dead and not SU.unit_dodges(store, target, true) then
	...
	while true do
		...
		if not (store.tick_ts - last_hit_ts >= this.aura.cycle_time) or this.aura.apply_duration and first_hit_ts and store.tick_ts - first_hit_ts > this.aura.apply_duration then
			...
			for i, target in ipairs(targets) do
				...
				if this.aura.nodes_limit and P:nodes_to_defend_point(target.nav_path) < this.aura.nodes_limit then
					goto label_899_0
				end
			...
			if this.aura.mod_tower then
				...
				if towers and #towers > 0 then
					for i = 1, #towers do
						local new_mod = E:create_entity(this.aura.mod_tower)

						new_mod.modifier.level = this.aura.level
						new_mod.modifier.target_id = towers[i].id
						new_mod.modifier.source_id = this.id
			...
			if this.aura.decal then
				local d = E:create_entity(this.aura.decal)
...
function scripts.aura_apply_damage.update(this, store, script)
	...
	if this.render and this.render.sprites[1].animated then
		while not U.animation_finished(this) do
			coroutine.yield()
		end
	end
...
function scripts.aura_unit_regen.update(this, store)
	while true do
		...
		if this.percentage then
			regen_health = math.ceil(target.health.hp_max * this.regen.percent)
		else
			regen_health = target.regen and target.regen.health or this.regen.health
		end
		
		if (this.regen.ignore_stun or not target.unit.is_stunned) and (this.regen.ignore_freeze or not U.has_modifier_types(store, target, MOD_TYPE_FREEZE)) and (this.regen.ignore_mods or not U.flag_has(target.vis.bans, F_MOD)) and regen_cooldown <= store.tick_ts - this.aura.ts then
...
function scripts.tunnel.update(this, store, script)
	...
	while true do
		...
		for _, enemy in pairs(enemies) do
			...
			local release_ts = tu.speed_factor and store.tick_ts + length / (tu.speed_factor * enemy.motion.max_speed)

			log.debug("tunnel %s picked %s", this.id, enemy.id)
			table.insert(picked_enemies, {
				release_ts = release_ts,
				entity = enemy,
				pick_ts = store.tick_ts
			})
		...
		for i = #picked_enemies, 1, -1 do
			...
			if p.release_ts and p.release_ts > store.tick_ts or tu.delay and store.tick_ts - p.pick_ts < tu.delay then
				-- block empty
			else
				if this.release_same then
					...
					local enemy = E:create_entity(tu.entity)
...
function scripts.mod_stun.insert(this, store, script)
	...
	if target and target.unit and this.render then
		for i = 1, #this.render.sprites do
			...
			elseif m.use_health_bar_offset then
				s.offset.x, s.offset.y = target.health_bar.offset.x, target.health_bar.offset.y
			end
		end
	end
...
function scripts.mod_hps.insert(this, store, script)
	...
	if not target or not this.modifier.revive and (not target.health or target.health.dead) then
		return false
	end

	if target.health.hp == target.health.hp_max and not this.modifier.keep_on_max then
		return false
	end
...
function scripts.mod_hps.update(this, store, script)
	...
	if hps.heal_percent then
		heal_min = target.health.hp_max * hps.factor
		heal_max = target.health.hp_max * hps.factor
	end
	...
	while true do
		...
		if not target or not this.modifier.revive and (not target.health or target.health.dead) or duration < store.tick_ts - m.ts then
			queue_remove(store, this)
		...
		if hps.heal_every and store.tick_ts - hps.ts >= hps.heal_every then
			...
			if this.modifier.revive and target.health.dead and not target.unit.hide_during_death then
...
function scripts.power_fireball.update(this, store, script)
	...
	local enemies = table.filter(store.entities, function(k, v)
		return v.unit and v.vis and v.health and not v.health.dead and band(v.vis.flags, b.damage_bans) == 0 and band(v.vis.bans, b.damage_flags) == 0 and U.is_inside_ellipse(v.pos, b.to, b.damage_radius)
	end)
...
scripts.power_fireball_tower = {}

function scripts.power_fireball_tower.update(this, store, script)
	local b = this.bullet
	local mspeed = 10 * FPS
	local particle = E:create_entity("ps_power_fireball")
...
if IS_KR1 or IS_KR2 then
	...
	function scripts.atomic_bomb.update(this, store)
		...
		local targets = table.filter(store.entities, function(k, v)
			return v.soldier and not v.hero and v.vis and v.health and not v.health.dead and band(v.vis.flags, b.damage_bans) == 0 and band(v.vis.bans, b.damage_flags) == 0
		end)
		...
		local towers = table.filter(store.entities, function(k, v)
			return not v.pending_removal and v.tower and not v.tower.blocked and v.tower.can_be_mod
		end)

		for _, tower in pairs(towers) do
			local m = E:create_entity("mod_fire_block")
...
scripts.power_nova_control = {}

function scripts.power_nova_control.can_select_point(this, x, y, store)
	return not GR:cell_is(x, y, TERRAIN_CLIFF) and (P:valid_node_nearby(x, y, 1.4285714285714286, NF_POWER_1) or store.level.fn_can_power and store.level:fn_can_power(store, GUI_MODE_POWER_1, V.v(x, y)) or GR:cell_is(x, y, TERRAIN_WATER))
end

function scripts.power_nova_control.update(this, store, script)
	local start_y = store.visible_coords and store.visible_coords.top or REF_H
	local ts
	local burst_interval = this.burst_interval

	for i = 1, math.max(this.fireball_count, this.cataclysm_count) do
		if i <= this.fireball_count then
			local e = E:create_entity("power_nova")
			local p, found, tries = nil, nil, 0
		...
		if i <= (this.cataclysm_count - 13) then
			local dest = P:get_random_position(10, bor(TERRAIN_LAND, TERRAIN_WATER))

			if dest then
				local e = E:create_entity("power_nova_small")
		...
		while (burst_interval - 0.34) > store.tick_ts - ts do
			coroutine.yield()
		end
		...
		while (burst_interval - 0.18) > store.tick_ts - ts do
			coroutine.yield()
		end
		
		if i <= (this.cataclysm_count - 14) then
			local dest = P:get_random_position(10, bor(TERRAIN_LAND, TERRAIN_WATER))
			...
			if dest then
				...
				e.bullet.to = V.vclone(dest)
				e.bullet.level = this.user_power.level
				e.sound_events.hit = "ElvesHeroVeznanArcaneNova"
...
function scripts.power_nova.update(this, store, script)
	local b = this.bullet
	local decal
	
	decal = E:create_entity("decal_power_nova_target")
	decal.pos = V.vclone(this.pos)
	decal.render.sprites[1].ts = store.tick_ts

	queue_insert(store, decal)
	S:queue(this.sound_events.hit, this.sound_events.hit_args)
	U.y_wait(store, b.wait_time)
	...
	for _, target in pairs(targets) do
		...
		u = UP:get_upgrade("rain_scorched_earth")
	...
	U.animation_start(sfx, nil, nil, store.tick_ts, false)
	queue_insert(store, sfx)
