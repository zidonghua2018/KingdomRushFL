...
local SU = require("script_utils_57")
	...
	while true do
		...
		if store.tick_ts - last_hit_ts >= this.aura.cycle_time then
			...
			for _, target in pairs(targets) do
				...
				if this.aura.hit_fx then
					local fx = E:create_entity(this.aura.hit_fx)

					fx.pos = V.vclone(target.pos)

					U.animation_start(fx, "idle", nil, store.tick_ts)
					queue_insert(store, fx)
				end
			end
		end
...
function scripts.mod_teleport.update(this, store)
	...
	if this.custom_tp_destination then
		local dest = this.custom_tp_destination
		local dest_pi, dest_spi

		if dest.use_unit_path and target.nav_path then
			dest_pi = {
				target.nav_path.pi
			}
			dest_spi = {
		...
		local node = P:nearest_nodes(dest.pos.x, dest.pos.y, dest_pi, dest_spi, true)[1]

		if target.nav_path then
			target.nav_path.pi = node[1]
			target.nav_path.spi = node[2]
			target.nav_path.ni = node[3]
		end

		local npos = P:node_pos(node[1], node[2], node[3])

		target.pos.x, target.pos.y = npos.x, npos.y
	elseif this.jump_connection then
	...
	elseif target.motion then
		target.nav_rally.center = V.vclone(target.pos)
		target.nav_rally.pos = target.nav_rally.center
	end
