...
function scripts.missile.update(this, store, script)
	...
	if b.damage_radius and b.damage_radius > 0 then
		...
		for _, enemy in pairs(enemies) do
			...
			if this.template_name == "missile_mecha" and shock_and_awe and band(enemy.vis.bans, F_STUN) == 0 and band(enemy.vis.flags, bor(F_BOSS, F_CLIFF, F_FLYING)) == 0 and math.random() < shock_and_awe.chance then
				local mod = E:create_entity("mod_shock_and_awe")
...
function scripts.power_reinforcements_control.insert(this, store, script)
	...
	local e = E:create_entity("re1_current_" .. i)
	...
	i = math.random(1, 3)
	e = E:create_entity("re1_current_" .. i)
	e.pos.x = x - 10
	e.pos.y = y + 10
	e.nav_rally.center = V.v(x, y)
	e.nav_rally.pos = V.vclone(e.pos)
