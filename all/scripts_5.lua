...
function scripts.mod_mark_flags.queue(this, store, insertion)
	...
	-- rime_light 这样做可以解决鳄鱼智者封塔卡死问题，但不确定是否会引入新的bug
...
function scripts.mod_teleport.update(this, store)
	...
	if this.jump_connection then
		...
		if this.dest_valid_node then
			local dpi, dni = P:find_valid_node(target.nav_path.pi, n_limit, this.dest_node_valid_dir or 1, this.dest_node_valid_flags or NF_ALL)
