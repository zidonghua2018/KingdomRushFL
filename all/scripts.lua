...
scripts.tower_hover_controller = {}

function scripts.tower_hover_controller.insert(this, store)
	local t = this.target

	if not t or not t.tower or not t.render.sprites[1] then
		return false
	end
...
function scripts.tower_hover_controller.update(this, store)
	local t = this.target
...
function scripts.tower_common.get_info(this)
	...
	if this.attacks and this.attacks.list[1].damage_min then
		...
		d_type = this.attacks.list[1].damage_type
...
scripts.tower_archer_kr = {}

function scripts.tower_archer_kr.insert(this, store, script)
	return true
end

function scripts.tower_archer_kr.update(this, store, script)
...
scripts.tower_archer_krf = {}

function scripts.tower_archer_krf.insert(this, store, script)
	return true
end

function scripts.tower_archer_krf.update(this, store, script)
	local at = this.attacks
	local a = this.attacks.list[1]
	local shooter_sprite_ids = table.slice({
		3,
...
scripts.soldier_barrack_krf = {}

function scripts.soldier_barrack_krf.update(this, store, script)
	local brk, sta
...
function scripts.bomb_krf.update(this, store, script)
	local b = this.bullet
...
function scripts.missile_krf.update(this, store, script)
	local b = this.bullet
...
function scripts.bolt_krf.update(this, store, script)
	local b = this.bullet
	local s = this.render.sprites[1]
	local mspeed = b.min_speed
	local target, ps
...
scripts.tower_archer_kro = {}

function scripts.tower_archer_kro.insert(this, store, script)
	return true
end

function scripts.tower_archer_kro.update(this, store, script)
	local at = this.attacks
	local a = this.attacks.list[1]
	local shooter_sprite_ids = table.slice({
		3,
...
function scripts.bomb_kr.update(this, store, script)
	local b = this.bullet
	local dmin, dmax = b.damage_min, b.damage_max
	local dradius = b.damage_radius
...
function scripts.bolt.update(this, store, script)
	...
	if b.hit_fx then
		...
		if target and b.hit_fx_flying and band(target.vis.flags, F_FLYING) ~= 0 then
			sfx.render.sprites[1].offset = this.custom_offsets.flying
		end
...
function scripts.shotgun_kr.insert(this, store, script)
	local b = this.bullet
...
function scripts.shotgun_kr.update(this, store, script)
	local b = this.bullet
	local target = store.entities[b.target_id]
	local speed = b.min_speed
	local target_invalid = false
...
function scripts.mod_track_target.insert(this, store, script)
	...
	if target and target.unit and target.render and this.render then
		for i = 1, #this.render.sprites do
			local s = this.render.sprites[i]
...
function scripts.mod_teleport.update(this, store)
	...
	local terrain_type0 = band(GR:cell_type(fx.pos.x, fx.pos.y), TERRAIN_TYPES_MASK)
	...
	local terrain_type = band(GR:cell_type(npos.x, npos.y), TERRAIN_TYPES_MASK)
	...
	if (target.water or target.cliff) and (terrain_type ~= terrain_type0) then
	...
	if pm.transfer_speed_factor and target and target.motion and e and e.motion then
		...
		if has then
			for _, m in pairs(mods) do
				if m.fast then
					e.motion.max_speed = e.motion.max_speed / (m.fast.factor or 1)
				elseif m.slow then
					e.motion.max_speed = e.motion.max_speed / (m.slow.factor or 1)
				end
			end
		end
	end
...
function scripts.editor_tower.insert(this, store)
	...
	--	this.render.sprites[2].name = string.format(this.render.sprites[1].name, store.level_terrain_type or 1)
	--end
