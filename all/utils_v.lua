...
function U.find_enemy_group(entities, origin, min_range, max_range, pair_range, flags, bans, min_count, filter_func, min_override_flags)
	...
	if not enemies or #enemies == 0 then
		...
		for i = 1, #enemies do
			local targets = U.find_enemies_in_range(entities, enemies[i].pos, 0,  pair_range, flags, bans, filter_func)
			if targets and #targets and #targets >= min_count then
				return enemies[i]
			else
				--block empty
			end
...
--[[
function U.find_nodes_in_range(origin, min_range, max_range, any_spi, banned_pi)
	local nodes = P:get_all_valid_pos(origin.x, origin.y, min_range, max_range, TERRAIN_LAND, nil, NF_RALLY, any_spi, banned_pi)
	local node_pos
	...
	if nodes and #nodes > 0 then
		for i = 1, #nodes do
			if nodes[i] then
				...
				if U.is_inside_ellipse(node_pos, origin, max_range) and (min_range == 0 or not U.is_inside_ellipse(node_pos, origin, min_range)) then
					table.insert(nodes_in_range, nodes[i])
				end
			else
				break
			end
...
---腐毒蘑菇
function U.find_nodes_in_range(origin, min_range, max_range, any_spi, banned_pi)
	local nodes = P:get_all_valid_pos_v(origin.x, origin.y, min_range, max_range, TERRAIN_LAND, nil, NF_RALLY, any_spi, banned_pi)
	local node_pos
	local nodes_in_range = {}
	local pi, spi, ni
...
function U.find_towers_in_range(entities, origin, attack, filter_func)
	local towers = table.filter(entities, function(k, v)
		return not v.pending_removal and v.tower and not v.tower.blocked and (not attack.excluded_templates or not table.contains(attack.excluded_templates, v.template_name)) and (not attack.max_range or U.is_inside_ellipse(v.pos, origin, attack.max_range)) and (not attack.radius or U.is_inside_ellipse(v.pos, origin, attack.radius)) and (not attack.min_range or attack.min_range == 0 or not U.is_inside_ellipse(v.pos, origin, attack.min_range)) and (not filter_func or filter_func(v, origin, attack))
	end)
...
function U.attack_order(attacks)
	...
	table.sort(order, function(o1, o2)
		...
		elseif o1.order_id and o2.order_id and o1.order_id ~= o2.order_id then
...
function U.predict_damage(entity, damage)
	...
	if e.health.true_armor and band(d.damage_type, DAMAGE_PHYSICAL) ~= 0 then
		rounded_damage = math.max(rounded_damage - e.health.true_armor, 0)
	end
