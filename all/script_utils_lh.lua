...
local function y_hero_walk_waypoints(store, this, animation)
	...
	while not V.veq(this.pos, dest) do
		local w = dest
		if n.waypoints then
			w = table.remove(n.waypoints, 1) or dest
		end
		--原：local w = table.remove(n.waypoints, 1) or dest
		local unsnap = n.waypoints and #n.waypoints > 0 or false
...
local function y_hero_death_and_respawn_lucerna(store, this)
	local h = this.health
	local he = this.hero

	this.ui.can_click = false
	this.render.sprites[7].hidden = true
	this.render.sprites[8].hidden = true
	...
	elseif band(h.last_damage_types, bor(DAMAGE_HOST)) ~= 0 then
		...
		S:queue(this.sound_events.death, this.sound_events.death_args)
		--U.y_animation_play(this, "death", nil, store.tick_ts, 1)
		U.animation_start_group(this, "death", nil, store.tick_ts, true, "layers")
	end
	...
	S:queue(this.sound_events.respawn)
	--U.y_animation_play(this, "respawn", nil, store.tick_ts, 1)
	U.animation_start_group(this, "respawn", nil, store.tick_ts, false, "layers")
...
local function soldier_idle_lucerna(store, this, force_ts)
	for i = 1,6 do
		U.animation_start(this, this.idle_flip.last_animation, nil, store.tick_ts, this.idle_flip.loop, i, force_ts)
	end
	if this.unit.is_stunned then
		return
	end
...
local SU = {
	...
	y_hero_death_and_respawn_lucerna = y_hero_death_and_respawn_lucerna,
	soldier_idle_lucerna = soldier_idle_lucerna,
	y_hero_new_rally_lucerna = y_hero_new_rally_lucerna,
