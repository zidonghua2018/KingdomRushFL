...
local function remove_modifiers(store, entity, mod_name, exclude_name)
	local mods = table.filter(store.entities, function(k, v)
		return entity and v.modifier and v.modifier.target_id == entity.id and (not mod_name or mod_name == v.template_name) and (not exclude_name or exclude_name ~= v.template_name)
	end)
...
local function do_death_spawns(store, this, id)
	if this.death_spawns.list[id].fx then
		local fx = E:create_entity(this.death_spawns.list[id].fx)

		fx.pos = V.vclone(this.pos)
		fx.render.sprites[1].ts = store.tick_ts

		if this.death_spawns.list[id].fx_flip_to_source and this.render and this.render.sprites[1] then
	...
	for i = 1, this.death_spawns.list[id].quantity do
		local s = E:create_entity(this.death_spawns.list[id].name)

		s.pos = V.vclone(this.pos)

		if this.death_spawns.list[id].spawn_animation and s.render then
			s.render.sprites[1].name = this.death_spawns.list[id].spawn_animation
		end
		...
		if s.nav_path then
			...
			local spread_nodes = this.death_spawns.list[id].spread_nodes
		...
		if this.death_spawns.list[id].offset then
			s.pos.x = s.pos.x + this.death_spawns.list[id].offset.x
			s.pos.y = s.pos.y + this.death_spawns.list[id].offset.y
		end
...
local function create_pop(store, pos, pop, custom_offset)
	local name = pop[math.random(1, #pop)]
	local e = E:create_entity(name)

	if custom_offset then
		e.pos = V.v(pos.x, pos.y + custom_offset)
	else
...
local function unit_dodges(store, this, ranged_attack, attack, source)
	...
	if not this.unit.is_stunned and (not this.dodge.requires_magic or this.enemy and this.enemy.can_do_magic) and (not ranged_attack or this.dodge.ranged) and (not this.dodge.cooldown or store.tick_ts - this.dodge.ts > this.dodge.cooldown) and (not attack or not attack.damage_type or band(attack.damage_type, DAMAGE_NO_DODGE) == 0) and math.random() <= this.dodge.chance and (not this.dodge.can_dodge or this.dodge.can_dodge(store, this, ranged_attack, attack, source)) then
		...
		if this.dodge.pop then
			local pop = create_pop(store, this.pos, this.dodge.pop, this.dodge.pop_offset)
...
local function parabola_y(phase, from_y, to_y, max_y)
	...
	b = not (not (M < C) and not (C < 0.0001)) and 1 or 2 / C * (M - math.sqrt(M * M - M * C))
...
local function y_hero_new_rally(store, this)
	...
	if r.new then
		...
		if hero_will_teleport(this, r.pos) then
			...
			if screen_map.movement_immunity == 1 then

				this.vis.bans = F_ALL
				this.health.ignore_damage = true
			else
				this.vis.bans = F_RANGED
			end
			...
			if tp.mod_on_arrival then
				local targets = U.find_targets_in_range(store.entities, this.pos, 0, tp.mod_range, tp.mod_flags, tp.mod_bans, function(v)
					return v ~= this
				end)

				if targets then
					if tp.max_targets then
						local affected = U.find_targets_in_range(store.entities, this.pos, 0, 9999, 0, 0, function(v)
							return v ~= this and U.has_modifiers(store, v, tp.mod) and not U.is_inside_ellipse(v.pos, this.pos, tp.mod_range)
						end)
						if affected and #affected + #targets > tp.max_targets then
							local count = (#affected + #targets) - tp.max_targets
							for i = 1, count do
								remove_modifiers(store, affected[i], tp.mod)
							end
						end
						for i = 1, math.min(#targets, tp.max_targets) do
							local target = targets[i]
							local m = E:create_entity(tp.mod)
...
local function y_hero_death_and_respawn(store, this)
	...
	if this.hero.skills.ultimate.is_transformation then
		local u = E:get_template(this.hero.skills.ultimate.controller_name)
		u.disabled = true
	end
	...
	if this.hero.skills.ultimate.transformed then
		this.render.sprites[1].prefix = this.hero.skills.ultimate.transform_prefix
		this.render.sprites[1].offset = this.hero.skills.ultimate.transform_offset
		this.hero.skills.ultimate.transformed = nil
		if this.hero.skills.ultimate.damage_factor then
			this.unit.damage_factor = this.unit.damage_factor / this.hero.skills.ultimate.damage_factor
		end
		if this.hero.skills.ultimate.bonus_armor then
			this.health.armor = this.health.armor - this.hero.skills.ultimate.bonus_armor
		end
		if this.hero.skills.ultimate.health_factor then
			this.health.hp_max = this.health.hp_max / this.hero.skills.ultimate.health_factor
			this.health.hp =  math.min(this.health.hp / this.hero.skills.ultimate.health_factor, this.health.hp_max)
		end
	end
...
local function y_soldier_death(store, this)
	...
	if this.death_spawns then
		for i = 1, #this.death_spawns.list do
			can_spawn[i] = this.death_spawns and this.death_spawns.list[i].enabled and band(this.health.last_damage_types, bor(DAMAGE_EAT, DAMAGE_NO_SPAWNS, this.death_spawns.list[i].no_spawn_damage_types or 0)) == 0
		end
	end
	
	if can_spawn and #can_spawn > 0 then
		for i = 1, #can_spawn do
			if can_spawn[i] and this.death_spawns.list[i].concurrent_with_death then
				do_death_spawns(store, this, i)
				coroutine.yield()
	...
	elseif this.reinforcement and this.reinforcement.fade_out then
		...
		if can_spawn and #can_spawn > 0 then
			for i = 1, #can_spawn do
				if can_spawn[i] and this.death_spawns.list[i].delay then
					while store.tick_ts - death_ts < this.death_spawns.list[i].delay do
						coroutine.yield()
					end
					do_death_spawns(store, this, i)
...
local function y_soldier_do_ranged_attack(store, this, target, attack, pred_pos)
	...
	elseif attack.max_track_distance and V.dist(target.pos.x, target.pos.y, bullet_to_start.x, bullet_to_start.y) > attack.max_track_distance then
		...
		bullet.bullet.pred_pos = pred_pos
		...
		if attack.shoot_fx then
			local fx = E:create_entity(attack.shoot_fx)
			
			local bb = bullet.bullet

			fx.pos.x, fx.pos.y = bb.from.x, bb.from.y
				
			if bb.to.x > this.pos.x then
				fx.render.sprites[1].offset = V.v(5, 0)
			else
				fx.render.sprites[1].offset = V.v(-5, 0)
			end
...
local function soldier_pick_ranged_target_and_attack(store, this)
	...
	for _, i in pairs(this.ranged.order) do
		...
		elseif a.sync_animation and not this.render.sprites[1].sync_flag then
			...
			if a.min_count then
				target = U.find_enemy_group(store.entities, this.pos, a.min_range, a.max_range, a.group_range, a.vis_flags, a.vis_bans, a.min_count, a.filter_fn, F_FLYING)
			else
...
local function y_soldier_do_single_melee_attack(store, this, target, attack)
	...
	if table.contains(target.enemy.blockers, this.id) then
		if unit_dodges(store, target, false, attack, this) then
			--
		else
			if attack.damage_type ~= DAMAGE_NONE then
				...
				if attack.mod_on_kill and U.predict_damage(target, d) >= target.health.hp then
					local m = E:create_entity(attack.mod_on_kill)
			...
			if attack.hit_fx then
				...
				for i = 1, #fx.render.sprites do
					...
					if attack.flip_fx then
						fx.render.sprites[i].flip_x = target.pos.x < this.pos.x
					end
				end
	...
	if attack.retreat then
		while not U.animation_finished(this) do
			coroutine.yield()
		end
		local tp = this.teleport
		local vis_bans = this.vis.bans
		local end_node = P:get_defend_point_node(target.nav_path.pi) or P:get_end_node(target.nav_path.pi)
		local r = P:node_pos(target.nav_path.pi, target.nav_path.spi, math.min(target.nav_path.ni + 15, end_node))
	...
	if this.dodge and this.dodge.counter_attack_pending then
		...
		for _, i in pairs(this.melee.order) do
			do
				...
				if not a.disabled and cooldown < store.tick_ts - a.ts and forced_cooldown_ok and band(a.vis_flags, target.vis.bans) == 0 and band(a.vis_bans, target.vis.flags) == 0 and (not a.fn_can or a.fn_can(this, store, a, target)) and (not a.not_first or this.melee.last_attack and this.melee.last_attack.target_id == target.id) and (not a.cooldown_same or not U.has_modifiers(store, target, a.mods[2])) then
...
local function can_melee_blocker(store, this, blocker)
	return not this.health.dead and not this.unit.is_stunned and this.melee and blocker and not blocker.health.dead and table.contains(this.enemy.blockers, blocker.id) and store.entities[blocker.id]
end
...
local function y_enemy_death(store, this)
	...
	if this.is_hitting_tower or this.tower_blocker then
		this.is_hitting_tower = false
		this.tower_blocker.blocked = nil
	end
	...
	if this.death_spawns then
		for i = 1, #this.death_spawns.list do
			can_spawn[i] = this.death_spawns and band(this.health.last_damage_types, bor(DAMAGE_EAT, DAMAGE_NO_SPAWNS, this.death_spawns.list[i].no_spawn_damage_types or 0)) == 0
		end
	end
...
local function y_enemy_walk_until_blocked(store, this, ignore_soldiers, func)
	...
	while ignore_soldiers or not blocker and not ranged do
		...
		if this.guardian then
			local enemies = U.find_enemies_in_range(store.entities, this.pos, 0, 9999, 0, 0, function(e)
				return e.id ~= this.id and e.template_name == this.template_name and e.health and not e.health.dead
			end)
		...
		if this.enemy.builder then
			if not blocker and not ignore_soldiers and not this.tower_blocker then
			local twr = table.filter(store.entities, function(k, v)
			return v and v.building and not v.pending_removal and U.is_inside_ellipse(v.pos, this.pos, this.enemy.builder_range) and table.contains(this.enemy.builder_templates, v.template_name) and not v.blocked
		end)
				if twr and #twr > 0 then
					local target = twr[1]
					local tx = target.pos.x + (target.pos.x > this.pos.x and -30 or 30)
					this.motion.forced_waypoint = V.v(tx, target.pos.y)
					this.vis.bans = bor(F_BLOCK)
					this.tower_blocker = target
					this.tower_blocker_ni = this.nav_path.ni
					ignore_soldiers = true
					target.blocked = true
				end
			elseif this.tower_blocker and this.motion.forced_waypoint == nil then
				this.is_hitting_tower = true
				this.vis.bans = 0
				return true, this.tower_blocker, ranged
			end
		end

		if ignore_soldiers or not blocker and not ranged then
			if this.run_particles_name then
				local particles = store.entities[this.particles_id]
				if particles then
					particles.particle_system.emit = true
				end
			end
...
local function y_wait_for_blocker(store, this, blocker)
	local pos 
	if blocker.motion then
		pos = blocker.motion.arrived and blocker.pos or blocker.motion.dest
	elseif blocker.building then
		pos = blocker.pos
	end
...
local function y_enemy_do_ranged_attack(store, this, target, attack)
	...
	if band(target.vis.bans, attack.vis_flags) == 0 and band(target.vis.flags, attack.vis_bans) == 0 then
		...
		if attack.twin_shot and math.random() < attack.twin_shot_chance then
...
local function y_enemy_do_loopable_ranged_attack(store, this, target, attack)
	...
	for i = 1, attack.loops do
		...
		for si, st in pairs(shoot_times) do
			...
			b.bullet.from = V.vclone(b.pos)
			if attack.launch_vector then
				b.bullet.to = V.v(b.pos.x + (af and -1 or 1) * attack.launch_vector.x, b.pos.y + attack.launch_vector.y)
			else
				b.bullet.to = V.v(target.pos.x + target.unit.hit_offset.x, target.pos.y + target.unit.hit_offset.y)
			end
			b.bullet.target_id = target.id
...
local function y_enemy_melee_attacks(store, this, target)
	for _, i in ipairs(this.melee.order) do
		...
		local mitarget = store.entities[target.id]
		...
		if not ma.disabled and cooldown <= store.tick_ts - ma.ts and band(ma.vis_flags, target.vis.bans) == 0 and band(ma.vis_bans, target.vis.flags) == 0 and (not ma.fn_can or ma.fn_can(this, store, ma, target)) then
			...
			if math.random() >= ma.chance then
				...
				for i = 1, #hit_times do
					...
					if ma.type == "melee" and (table.contains(this.enemy.blockers, target.id) or screen_map.movement_immunity == 0 and mitarget and not mitarget.building and not mitarget.dead) then
						if dodged then
							--
						else
							local d = E:create_entity("damage")
							...
							elseif ma.damage_min then
								...
								if ma.damage_charge then
									ma.damage_max = ma.damage_max + ma.damage_charge
									ma.damage_min = ma.damage_min + ma.damage_charge
								end
					...
					elseif ma.type == "melee" and ma.build then
						target.build_progress = (target.build_progress or 0) + 1
						if target.build_progress == target.max_hits then
							this.tower_blocker = nil
							this.is_hitting_tower = false
							this.nav_path.ni = this.tower_blocker_ni + 10
						end
					...
					elseif ma.type == "bullet" then
						local e = E:create_entity(ma.bullet)
						e.bullet.source_id = this.id
						e.bullet.target_id = target.id
						e.bullet.from = V.v(this.pos.x + ma.bullet_start_offset.x, this.pos.y + ma.bullet_start_offset.y)
						e.bullet.to = V.v(target.pos.x, target.pos.y)
						if e.bullet.hit_fx then
					...
					if ma.aura then
						local aura = E:create_entity(ma.aura)

						aura.pos = V.vclone(hit_pos)
				...
				--[[if screen_map.movement_immunity == 1 then
					if #this.enemy.blockers == 0 then
						...
						if #this.enemy.blockers == 1 and blocker.motion and not blocker.motion.arrived then
							ma.ts = 0
						end
					end
				end]]--
...
local function y_enemy_mixed_walk_melee_ranged(store, this, ignore_soldiers, walk_break_fn, melee_break_fn, ranged_break_fn)
	...
	if blocker then
		if ranged and this.ranged_in_melee then
			while can_range_soldier(store, this, ranged) and (not ranged_break_fn or not ranged_break_fn(store, this)) do
